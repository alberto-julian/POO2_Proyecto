...
MyWindow::MyWindow():RenderWindow(VideoMode(LADO,LADO),"Simulador de Particulas") {
    ...
    while(isOpen()) {
        ...
        for(int i = 0; i < Particulas.size(); i++) {
            ...
            for(int j = i+1; j < Particulas.size(); j++) {
                if(en_contacto(Particulas[i], Particulas[j])) {
                    if(Particulas[i].get_estado() == color::celeste && Particulas[j].get_estado() == color::amarillo)
                        Particulas[i].set_estado(color::amarillo);
                    if(Particulas[i].get_estado() == color::amarillo && Particulas[j].get_estado() == color::celeste)
                        Particulas[j].set_estado(color::amarillo);
                }
            }
        }
        display();
