...
void Particula::estado_choque_particula(Particula& a){
    if(estado == color::celeste && a.estado == color::amarillo)
        estado=color::amarillo;
    if(estado == color::amarillo && a.estado == color::celeste)
        a.estado=color::amarillo;
}

void Particula::set_angulo_particula(Particula& a) {
    if(estado==color::rojo && a.estado!=color::rojo){
        if(a.angulo<=180)
            a.angulo=180+a.angulo;
        else
            a.angulo = a.angulo - 180;
    }
    if(estado!=color::rojo && a.estado==color::rojo){
        if(angulo<=180)
            angulo=180+angulo;
    ...
    if(estado!=color::rojo && a.estado!=color::rojo){
        swap(angulo,a.angulo);
    }
    movimiento();
    //this->movimiento();
}

void Particula::choque_particula(Particula& a){
    if(en_contacto(*this, a)) {
        estado_choque_particula(a);
        //estado_choque_particula(a);
        set_angulo_particula(a);
        //set_angulo_particula(a);
    }
}

void Particula::set_color_particula() {
    switch(estado) {
        case color::celeste:
...
void Particula::choque_pared() {
    if(posicionx+2*RADIO>=LADO || posicionx<=0)
        if(angulo<=180)
            angulo=180-angulo;
        else
            angulo=540-angulo;
    else if(posiciony+2*RADIO>=LADO || posiciony<=0)
        angulo = 360-angulo;
}
