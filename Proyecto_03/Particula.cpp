...
void Particula::choque_pared() {
    if(posicionx>=(LADO-2*RADIO) || posicionx<=0){//Derecha e izquierda
        if(angulo<=180)
            angulo=180-angulo;//
        else{
            angulo = angulo - 180;
    ...
    else if(posiciony>=(LADO-2*RADIO) || posiciony<=0)//Inferior y superior
        angulo=360-angulo;
}

void Particula::cambio_color_estado() {
    switch(estado) {
        case color::celeste:
...
void Particula::movimiento() {
    if(estado!=color::rojo){
        ...
        if(aumentoy<0.0001 && aumentoy>-0.0001) aumentoy=0;
        if(aumentox<0.0001 && aumentox>-0.0001) aumentox=0;
        posicionx+=aumentox;
        posiciony+=aumentoy;
